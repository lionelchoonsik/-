...
특정 상황에서 덱 대신 큐와 스택을 사용하는 이유는 큐와 스택의 속성이 다르고 사용 사례가 다르기 때문입니다. 예를 들어 대기열은 FIFO(선입선출) 순서를 유지하려는 시나리오에서
종종 사용됩니다. 이것은 버스를 기다리는 사람들의 줄을 시뮬레이션하는 데 유용할 수 있으며, 여기서 줄을 선 첫 번째 사람이 버스에 탑승할 것입니다. 이와 반대로 스택은
LIFO(라스트 인 퍼스트 아웃) 순서를 유지하려는 시나리오에서 종종 사용됩니다. 
따라서 특정 사용 사례에 따라 대기열 또는 스택을 선택하고 해당 데이터 구조에 적합한 작업을 사용합니다.
